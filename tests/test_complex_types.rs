...
fn test_tuple_return_type() {
    ...
    assert_eq!(get_tuple_data.return_type, "[string, number, number | null]");
}
...
fn test_zod_schema_generation_for_maps() {
    ...
    assert_eq!(schema, "z.record(z.string(), z.string())");
...
fn test_zod_schema_generation_for_tuples() {
    ...
    assert_eq!(schema, "z.tuple([z.string(), z.number(), z.boolean().nullable()])");
...
fn test_enum_variant_parsing() {
    ...
    fs::copy("tests/fixtures/complex_types.rs", temp_dir.path().join("complex_types.rs")).unwrap();
    let _ = analyzer.analyze_project(temp_dir.path().to_str().unwrap()).unwrap();
...
fn test_full_generation_with_complex_types() {
    ...
    let mut analyzer = CommandAnalyzer::new();
    let commands = analyzer.analyze_file(Path::new("tests/fixtures/complex_types.rs")).unwrap();
    // Create a temp directory with only our complex_types.rs file to avoid syntax errors
    let temp_dir_for_analysis = TempDir::new().unwrap();
    fs::copy("tests/fixtures/complex_types.rs", temp_dir_for_analysis.path().join("complex_types.rs")).unwrap();
