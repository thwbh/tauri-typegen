...
"#;
    ...
    // Should be: z.string().max(500, { message: "..." }).optional() NOT z.string().optional().max(500)
...
fn test_validation_error_messages() {
    ...
    // Create a test file with validator attributes that include custom error messages
    let test_content = r#"
use serde::{Deserialize, Serialize};
use validator::Validate;
use tauri::command;
...
#[derive(Debug, Deserialize, Validate)]
pub struct FormData {
    #[validate(length(min = 5, max = 50, message = "Username must be between 5 and 50 characters"))]
    pub username: String,
...
#[command]
pub async fn submit_form(data: FormData) -> Result<String, String> {
