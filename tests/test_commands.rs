...
async fn test_generate_models_success() {
    let temp_dir = create_test_project_with_commands();
    let project_path = temp_dir.path().to_str().unwrap().to_string();
    let output_path = temp_dir.path().join("generated").to_str().unwrap().to_string();
...
async fn test_generate_models_without_validation() {
    ...
    assert!(response.generated_files.contains(&"commands.ts".to_string()));
...
async fn test_generate_models_creates_output_directory() {
    let temp_dir = create_test_project_with_commands();
    let project_path = temp_dir.path().to_str().unwrap().to_string();
    let output_path = temp_dir.path().join("deep/nested/output").to_str().unwrap().to_string();
...
async fn test_command_detail_extraction() {
    ...
    let create_user = response.commands.iter()
    ...
    let calculate = response.commands.iter()
