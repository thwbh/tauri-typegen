...
pub fn ping(payload: PingRequest) -> Result<PingResponse> {
    Ok(PingResponse {
        value: Some(format!("Pong: {}", payload.value.unwrap_or_else(|| "Unknown".to_string()))),
    })
}

pub async fn analyze_commands(
    payload: AnalyzeCommandsRequest,
) -> Result<AnalyzeCommandsResponse> {
...
pub async fn generate_models(
    payload: GenerateModelsRequest,
) -> Result<GenerateModelsResponse> {
    let mut analyzer = CommandAnalyzer::new();
    
    // Analyze commands first
    let commands = analyzer.analyze_project(&payload.project_path)
    ...
    let output_path = payload.output_path.unwrap_or_else(|| {
        format!("{}/generated", payload.project_path)
    });
    
    let generated_files = generator.generate_models(&commands, analyzer.get_discovered_structs(), &output_path)
